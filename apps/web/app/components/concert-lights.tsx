...
export function ConcertLights() {
  ...
  useEffect(() => {
    ...
    const initLights = () => {
      ...
      for (let i = 0; i < 2; i++) {
        lightBeams.push({
          ...
          angle: i === 0 ? Math.random() * Math.PI/4 + Math.PI/8 : Math.PI - Math.random() * Math.PI/4 - Math.PI/8,
          color: lightColors[Math.floor(Math.random() * lightColors.length)],
    ...
    const drawBeam = (beam: LightBeam) => {
      ...
      if (beam.type === 'laser') {
        // Sharp laser beam
        gradient.addColorStop(0, `${beam.color}${Math.floor(beam.intensity * 255).toString(16).padStart(2, '0')}`);
        gradient.addColorStop(0.7, `${beam.color}${Math.floor(beam.intensity * 128).toString(16).padStart(2, '0')}`);
        gradient.addColorStop(1, `${beam.color}00`);
      } else {
        // Softer spotlight cone
        gradient.addColorStop(0, `${beam.color}${Math.floor(beam.intensity * 180).toString(16).padStart(2, '0')}`);
        gradient.addColorStop(0.3, `${beam.color}${Math.floor(beam.intensity * 100).toString(16).padStart(2, '0')}`);
        gradient.addColorStop(1, `${beam.color}00`);
      }
