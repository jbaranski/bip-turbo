...
import type { MatchDetails } from "@bip/domain";
...
function SearchResult({ result }: SearchResultItemProps) {
  ...
  const formatMatchDetails = (details: string) => {
    ...
    try {
      ...
      if (parsed.type === 'trackMatches' && parsed.tracks) {
        // Format track matches into readable context
        const contextParts = parsed.tracks.map((track) => {
          let context = '';
          ...
          if (track.note && musicalAnnotations.some(annotation => 
        ...
        return contextParts.length > 1 
          ? contextParts.join('\n') 
          : contextParts[0];
      }
      
      if (parsed.type === 'segue' && parsed.song1 && parsed.song2) {
        // Format segue matches
        let context = `${parsed.song1} ${parsed.segueSymbol} ${parsed.song2}`;
        if (parsed.set) {
          context = `${parsed.set}: ${context}`;
        }
        return context;
      }
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        {result.score && (
          <span className="text-xs text-purple-400 font-medium">{result.score}%</span>
        )}
      </div>
    </div>
  );
...
export function GlobalSearchDrawer({ open, onOpenChange }: GlobalSearchDrawerProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-lg lg:max-w-xl xl:max-w-2xl overflow-y-auto bg-gradient-to-br from-gray-900 via-purple-900/40 to-gray-900 border-l border-purple-500/30 backdrop-blur-md">
        ...
        <div className="space-y-6">
          ...
          {!query.trim() && (
            <div className="py-8 text-center">
              ...
              <p className="text-sm text-gray-500 mt-2">Try searching for a date, venue, or song name</p>
            </div>
          )}
        </div>
      </SheetContent>
