...
>(({ asChild, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn("transition-colors hover:text-brand-primary text-content-text-tertiary", className)}
      {...props}
    />
  );
});
...
const BreadcrumbSeparator = ({ children, className, ...props }: React.ComponentProps<"li">) => (
  <li
    role="presentation"
    aria-hidden="true"
    className={cn("[&>svg]:w-3 [&>svg]:h-3 text-brand-primary/60", className)}
    {...props}
  >
