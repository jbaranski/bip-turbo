...
const ChemicalWarfareBrigade: React.FC = () => {
  ...
  return (
    <div className="w-full px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">The Chemical Warfare Brigade</h1>

      <Card title="Introduction">
        <CardContent>
          Chemical Warfare Brigade, Marc Brownstein's rock opera, was first debuted by his side project Electron on
          08/18/00 and then by The Disco Biscuits on 12/30/00. While a number of the songs were written during Marc's
          departure from the band in 2000, Plan B (10/16/98), Three Wishes (12/03/98), Little Lai (04/21/99), and
          Chemical Warfare Brigade (08/21/99) were all written and debuted prior to that.
        </CardContent>

        <CardContent>
          Performed in full six times by TDB, the most recent occurrence was on 12/31/13 at the Theater at Madison
          Square Garden in New York. After the opening tracks of Plan B and Little Lai, the opera was played out of
          order with songs like Hot Air Balloon, House Dog Party Favor and Helicopters interwoven into the setlist.
        </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        <div>
          <OperaMenuCard title="The Cast" items={chars.map((char) => ({ name: char.name, anchor: char.anchor }))} />
        </div>
        <div>
          <OperaMenuCard title="Overview" items={acts.map((act) => ({ name: act.name, anchor: act.anchor }))} />
        </div>
        <div>
          <OperaMenuCard title="Songs" items={songs.map((song) => ({ name: song.name, anchor: song.anchor }))} />
        </div>
      </div>
