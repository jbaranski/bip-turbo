...
BEGIN
  ...
  matched_tracks AS (
    -- Find tracks matching our search songs for segue searches
    SELECT 
      tp.show_id,
      tp.track_id,
      tp.position,
  ...
  show_matches AS (
    SELECT 
      ...
      CASE 
        WHEN sm.match_show_id IS NOT NULL THEN 100
        ELSE 50
      END as proximity_score,
      COALESCE(
        sm.set || ': ' || sm.segue_text,
        STRING_AGG(
          DISTINCT mt.song_title || 
          CASE 
    ...
    GROUP BY sm.match_show_id, mt.show_id, sm.set, sm.segue_text
  )
  SELECT 
    sm.match_show_id,
    sh.slug::TEXT,
