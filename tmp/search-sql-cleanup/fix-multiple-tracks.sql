-- Update the search_shows_with_songs_v2 function to handle songs that appear multiple times
...
BEGIN
  ...
  matched_tracks AS (
    -- Find ALL tracks matching our search songs (don't use DISTINCT here)
    SELECT 
      tp.*,
  ...
  show_matches AS (
    SELECT 
      ...
      -- Build match details showing all matching tracks
      STRING_AGG(
        mt.set || ': ' || mt.song_title || 
        CASE 
        ...
        END,
        CASE 
          WHEN mt.segue = '>' AND mt.next_track_id IN (SELECT track_id FROM matched_tracks WHERE show_id = mt.show_id) 
          THEN ' > ' 
          ELSE ', ' 
        END 
