-- Update search function to use AND logic when both venue_ids and song_ids are provided
...
BEGIN
  ...
  show_matches AS (
    ...
    WHERE 
      ...
      -- AND LOGIC: When both song_ids and venue_ids are provided, require BOTH matches
      AND (
        -- Case 1: Both song and venue IDs provided - require both matches
        (song_ids IS NOT NULL AND venue_ids IS NOT NULL AND 
         mt.song_match_score >= 50 AND vms.venue_match_score >= 50)
        OR
        -- Case 2: Only song IDs provided - just need song match
        (song_ids IS NOT NULL AND venue_ids IS NULL AND mt.song_match_score >= 30)
        OR
        -- Case 3: Only venue IDs provided - just need venue match  
        (song_ids IS NULL AND venue_ids IS NOT NULL AND vms.venue_match_score >= 50)
        OR
        -- Case 4: No entity IDs - fallback to lower threshold
        (song_ids IS NULL AND venue_ids IS NULL AND 
         (mt.song_match_score >= 20 OR vms.venue_match_score > 0))
      )
