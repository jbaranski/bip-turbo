-- Update the search_shows_with_songs_v2 function to improve match details display
...
BEGIN
  ...
  matched_tracks AS (
    ...
    SELECT DISTINCT ON (tp.track_id)
      tp.*,
  ...
  show_matches AS (
    SELECT 
      ...
      -- Build match details with cleaner formatting
      STRING_AGG(
        DISTINCT 
        ...
        CASE 
          ...
          ELSE ''
        END,
        CASE WHEN mt.segue = '>' THEN ' > ' ELSE ', ' END 
