-- Fix ambiguous column references in search_shows_with_songs_v2
  ...
  show_matches AS (
    SELECT 
      ...
      STRING_AGG(
        ...
        END,
        CASE 
          WHEN mt.segue = '>' AND EXISTS (
            SELECT 1 FROM matched_tracks mt_next 
            WHERE mt_next.track_id = mt.next_track_id 
            AND mt_next.show_id = mt.show_id
          )
          THEN ' > ' 
          ELSE ', ' 
        END 
