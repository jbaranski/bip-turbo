-- Fix match details formatting with proper set grouping and segues
...
BEGIN
  ...
  matched_tracks AS (
    -- Find ALL tracks matching our search songs
    SELECT 
      tp.show_id,
      tp.track_id,
      tp.position,
  ...
  show_matches AS (
    SELECT 
      ...
      -- Build match details grouped by set with proper segues
      STRING_AGG(
        CASE 
          WHEN LAG(mt.set) OVER (PARTITION BY mt.show_id ORDER BY mt.position) IS NULL 
            OR LAG(mt.set) OVER (PARTITION BY mt.show_id ORDER BY mt.position) != mt.set
          THEN mt.set || ': '
          ELSE ''
        END ||
        mt.song_title || 
  ...
  SELECT 
    ...
    v.country::TEXT,
    sm.proximity_score,
    -- Clean up the match details string
    REGEXP_REPLACE(sm.match_details::TEXT, ', $', '') as match_details
