...
async function indexSongs() {
  ...
  try {
    ...
    console.log(`💰 Estimated cost: $${totalEstimatedCost.toFixed(4)} (${Math.round(estimatedTokens * songs.length / 10)} tokens)`);
    ...
    for (let i = 0; i < songs.length; i += batchSize) {
      ...
      console.log(`📝 Indexing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(songs.length / batchSize)} (${batch.length} songs)...`);
