...
async function indexVenues() {
  ...
  try {
    ...
    console.log(
      `💰 Estimated cost: $${totalEstimatedCost.toFixed(4)} (${Math.round((estimatedTokens * venues.length) / 10)} tokens)`,
    );
    ...
    for (let i = 0; i < venues.length; i += batchSize) {
      ...
      console.log(
        `📝 Indexing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(venues.length / batchSize)} (${batch.length} venues)...`,
      );
