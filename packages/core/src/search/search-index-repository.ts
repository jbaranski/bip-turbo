...
export class SearchIndexRepository {
  ...
  async search(queryEmbedding: number[], options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    if (threshold > 0) {
      whereConditions.push(`1 - (${embeddingField} <=> $1::vector) >= $${++paramIndex}`);
    ...
    const results = await this.db.$queryRawUnsafe<SearchResult[]>(query, ...queryParams);
   ...
   */
  async findByEntity(entityType: string, entityId: string): Promise<SearchResult | null> {
