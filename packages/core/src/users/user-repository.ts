...
export class UserRepository {
  ...
  private calculateCommunityScore(reviewCount: number, attendanceCount: number, ratingCount: number, blogPostCount: number): number {
  ...
  async getTopUsersByMetric(metric: "reviews" | "attendance" | "ratings" | "blogPostCount", limit: number = 10): Promise<UserStats[]> {
    ...
    if (metric === "blogPostCount") {
      console.log(`DEBUG: Top bloggers after filter:`, filteredStats.slice(0, 3).map(s => `${s.user.username}: ${s.blogPostCount}`));
    }
